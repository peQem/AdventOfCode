...
x <- input %>% 
  ...
  mutate(across(.fns = ~ as.integer(.x)),
         ...
         overlap = map2(XX1, XX2, ~ intersect(.x, .y)),

         fully_contain = map2_lgl(XX1, overlap, ~ length(.x) == length(.y)) | map2_lgl(XX2, overlap, ~ length(.x) == length(.y))) 

# solution part 1
sum(x$fully_contain)

# solution part 2
sum(map_lgl(x$overlap, ~ length(.x) > 0))
